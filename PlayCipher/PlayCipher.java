...
public class PlayCipher {
    ...
    private String decryptPair(char a, char b) {
        ...
        if (aPos[0] == bPos[0]) {
            // Same row: Shift left.
            decrypt.append(matrix[aPos[0]][(aPos[1] + 4) % 5]);
            decrypt.append(matrix[bPos[0]][(bPos[1] + 4) % 5]);
        } else if (aPos[1] == bPos[1]) {
            // Same column: Shift up.
            decrypt.append(matrix[(aPos[0] + 4) % 5][aPos[1]]);
            decrypt.append(matrix[(bPos[0] + 4) % 5][bPos[1]]);
        } else {
            // Rectangle rule: Swap columns.
            decrypt.append(matrix[aPos[0]][bPos[1]]);
            decrypt.append(matrix[bPos[0]][aPos[1]]);
        }

        return decrypt.toString(); // Return the decrypted pair.
    }
